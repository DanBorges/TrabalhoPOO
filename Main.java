...
public class Main {
	...
	public static int carregarMenuPrincipal(Scanner in) {
		...
		int op = 0;
		/*
		 * Sai do loop somente quando a variável entradaValida for igual a true,
		 * fazendo com que o usuário tenha uma nova chance em escolher uma opção
		 * válida
		 */
		while (!entradaValida) {
			/* Menu Principal */
			...
			try {
				...
				/*
				 * Altera o valor de entradaValida, para que saia do loop se a
				 * opção digitada estiver no menu
				 */
			...
			try {
				...
				if (op == 1 || op == 2 || op == 3 || op == 4 || op == 5) {
					...
					if (op == 1) {
						...
						cpf = in.next();
						/*
						 * Verifica se não existe um funcionário com o mesmo
						 * CPF. Se Existir não executa bloco
						 */
						if (imobiliaria.buscarFuncionario(cpf) == null) {
							...
							/*
							 * Instancia funcionário como variável polimórfica
							 * para funcionário fixo
							 */
						...
						/*
						 * Caso o cpf já esteja cadastrado no sistema é
						 * disparado mensagem Já exixte um funcionário Fixo com
						 * esse CPF
						 */
						else {
					...
					if (op == 2) {
						...
						/*
						 * Caso o cpf já esteja cadastrado no sistema é
						 * disparado mensagem Já exixte um funcionário
						 * Comissionado com esse CPF
						 */
						else {
					...
					if (op == 3) {
						...
						cpfPesquisa = in.next();
						/*
						 * Faz a busca do funcionário pelo seu CPF, caso
						 * encontro imprime a lista de funcionários fixos
						 */
						if (imobiliaria.buscarFuncionarioFixo(cpfPesquisa) != null)
							System.out.println(imobiliaria.buscarFuncionarioFixo(cpfPesquisa));
						/*
						 * senão encontrar o funcionário na lista exibe a
						 * mensagem Não existe um funcionário Fixo com esse CPF
						 */
						else {
					...
					if (op == 4) {
						...
						/*
						 * Faz a busca do funcionário pelo seu CPF, caso
						 * encontro imprime a lista de funcionários autônomos
						 */
			...
			try {
				...
				if (op == 1 || op == 2 || op == 3 || op == 4) {
					...
					if (op == 1) {
						...
						/*
						 * Intancia objeto imóvel passando os atributos de
						 * endereço para ser instânciado dentro da classe Imovel
						 */
						Imovel imovel = new Imovel(0, "", 0, 0, false, cidade,
						rua, bairro, complemento);
						int idImovel;
						String nomeProprietario;
						Integer categoria, tipo;

						/*
						 * Para cada atributo lido de imóvel é setado o valor do
						 * imóvel
						 */
						...
						if(imobiliaria.buscarImovel(idImovel) != null){
							System.out.println("Já existe um Imóvel com esse Id");
							return;
						} 
						...
						/*
						 * Altera a categoria do imóvel se a categoria for
						 * residencial conforme o valor do enum definido na
						 * classe Imovel
						 */
						...
						/*
						 * Altera a categoria do imóvel se a categoria for
						 * comercial conforme o valor do enum definido na classe
						 * Imovel
						 */
						...
						/*
						 * Altera tipo do imóvel se o tipo for casa conforme o
						 * valor do enum definido na classe Imovel
						 */
					...
					if (op == 2) {
						...
						/*
						 * Imprime os imóveis que são da mesma categoria e tipo
						 * e cidade, informados pelo usuário
						 */
					...
					if (op == 3) {
						...
						/*
						 * Imprime os imóveis que são da mesma categoria e tipo
						 * e cidade e bairro, informados pelo usuário
						 */
	...
	public static void carregarModuloContrato(Imobiliaria imobiliaria) {
		...
		System.out.println("3. Rescindir Contrato");
		System.out.println("4. Voltar");
		/* Tenta Executar o bloco de comandos dentro de try */
		...
		op = Integer.parseInt(input);
		/*
		 * Altera o valor de entradaValida, para que saia do loop se a opção
		 * digitada estiver no menu
		 */
		if (op == 1 || op == 2 || op == 3 || op == 4) {
			entradaValida = true;
			if (op == 1) {
				...
				Funcionario funcionario = imobiliaria
						.buscarFuncionario(cpfFuncionario);
				/*
				 * Caso não encontre o funcionário exibe a mensagem Não Existe
				 * nenhum funcionário com esse CPF
				 */
				if (funcionario == null) {
					System.out
							.println("Não Existe nenhum funcionário com esse CPF");
					return;
				}
				...
				/*
				 * Caso não encontre o imóvel exibe a mensagem Não Existe nenhum
				 * Imóvel com esse Id
				 */
				if (imovel == null) {
					...
					/*
					 * Caso encontre um contrato com o mesmo número lido exibe a
					 * mensagem Não Existe nenhum Imóvel com esse Id
					 */
					...
					/*
					 * Chama a funcão para gerar o contrato na classe
					 * funcionário
					 */
					contrato = funcionario.gerarContrato(contrato, funcionario,
							imovel);
					/* Adiciona o contrato na lista de contratos */
					imobiliaria.adicionarContrato(contrato);

					/*
					 * lista de contratos do cpf lido, recebida como retorno do
					 * método buscarContratosPorCpf
					 */
					ArrayList<Contrato> listaContrato = imobiliaria.buscarContratosPorCpf(cpfFuncionario);

					/*
					 * Passa como parâmentro a lista de contratos do funcionário
					 * específico para calcuar sua remuneração
					 */
			...
			if (op == 2) {
				...
				numero = in.nextInt();
				/*
				 * Caso não encontre nenum contrato com o idContrato lido exibe
				 * a mensagem Não existe nenhum contrato com esse número
				 */
				if (imobiliaria.buscarContratos(numero) == null) {
			...
			if (op == 3) {
				...
				if (imobiliaria.buscarContratos(numero) == null) {
					System.out
							.println("Não existe nenhum contrato com esse número");
				} else {
					imobiliaria.rescindirContrato(numero);
					System.out.println("Contrato rescindido com Sucesso");
				}
			}
		}
	}
	 ...
	 * 
	 * @param args
	 * Função pricipal. Carrega o menu principal e aguarda a ação do usuário
	 */
	public static void main(String[] args) {
		...
		do {
			/*
			 * A variável opPrincipal recebe o retorno da função
